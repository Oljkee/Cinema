...
class UserPreferencesRepositoryImpl @Inject constructor(private val context: Context) : UserPreferencesRepository{
    ...
    override suspend fun updateOnBoardingCompleted(isCompleted: Boolean) {
        context.dataStore.edit { pref ->
            pref[PreferencesKeys.ONBOARDING_COMPLETED_KEY] = isCompleted
        }
    }

    override val isOnBoardingCompletedFlow: Flow<Boolean> = context.dataStore.data
        .map { pref ->
            pref[PreferencesKeys.ONBOARDING_COMPLETED_KEY] ?: false
        }

    override suspend fun updateSearchFilterParameters(filterParameters: FilterParameters) {
        context.dataStore.edit { pref ->
    ...
    override val searchFilterParameters: Flow<FilterParameters> = context.dataStore.data
        .map { pref ->
            FilterParameters(
